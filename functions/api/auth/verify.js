...
export async function onRequestPost(context) {
  ...
  const firstDevice = { id: genId('dev_', 10), name: 'Primary Device', userAgent: ua, created: now, lastSeen: now };
  ...
  const user = {
    ...
    created: now,
    emailVerified: true,
    profiles: [{ id: 'p1', name: pending.name || 'Captain', avatar: '\u{1F3F4}‍☠️', isOwner: true }],
    activeProfile: 'p1',
    devices: [firstDevice],
    onboarded: false,
  };
  ...
  return json({
    success: true, token,
    user: { id: userId, email: emailNorm, name: user.name, tier: user.tier, profiles: user.profiles, activeProfile: user.activeProfile, devices: user.devices },
