// POST /api/auth/link/approve
// Validates a link code and creates a linked session
import { json, preflight, genId, TIER_LIMITS } from '../../_shared.js';

const DEVICE_LIMITS = {
  free: 1, account: 2, premium: 4, pro: 6, ultra: 10,
};

export function onRequestOptions() { return preflight(); }

export async function onRequestPost(context) {
  const { request, env } = context;
  if (!env.SYNC_KV) return json({ error: 'KV not configured' }, 503);

  let body;
  try { body = await request.json(); } catch { return json({ error: 'Invalid JSON' }, 400); }

  const { code } = body;
  if (!code) return json({ error: 'Link code required' }, 400);

  const linkRaw = await env.SYNC_KV.get(`link:${code}`);
  if (!linkRaw) return json({ error: 'Invalid or expired link code' }, 400);

  const link = JSON.parse(linkRaw);
  const userRaw = await env.SYNC_KV.get(`user:${link.userId}`);
  if (!userRaw) return json({ error: 'Account not found' }, 404);

  const user = JSON.parse(userRaw);
  if (user.banned) return json({ error: 'Account banned' }, 403);

  // Device limit check
  const maxDevices = DEVICE_LIMITS[user.tier] || 1;
  const devices = user.devices || [];
  if (devices.length >= maxDevices) {
    return json({ error: `Device limit reached (${maxDevices}). Upgrade your tier to add more devices.` }, 403);
  }

  // Create a new session
  const token = genId('tok_', 24);
  await env.SYNC_KV.put(`session:${token}`, JSON.stringify({ userId: user.id, created: Date.now(), linked: true }), { expirationTtl: 90 * 24 * 3600 });

  // Track device
  const deviceId = genId('dev_', 10);
  const ua = request.headers.get('User-Agent') || 'Unknown';
  devices.push({ id: deviceId, name: `Device ${devices.length + 1}`, userAgent: ua.slice(0, 120), created: Date.now(), lastSeen: Date.now() });
  user.devices = devices;

  await env.SYNC_KV.put(`user:${user.id}`, JSON.stringify(user));
  await env.SYNC_KV.delete(`link:${code}`);

  return json({
    success: true, token, linked: true,
    user: { id: user.id, email: user.email, name: user.name, tier: user.tier, profiles: user.profiles, activeProfile: user.activeProfile },
    tierLimits: TIER_LIMITS[user.tier],
  });
}
